...
class NaturalEarth:
    ...
    def get_lores_world_boundaries(self) -> Tuple[gpd.GeoDataFrame, NDArray]:
        ...
        self.world_lores = gpd.read_file(gpd.datasets.get_path("naturalearth_lowres"))\
                              .to_crs("EPSG:4087")
...
class GridSearch:
    ...
    def plot_grid(
        ...
        if bbox is None:
            bbox = [coords.latitude.min(), coords.longitude.min(), coords.latitude.max(),
        ...
        if countries is not None:
            ...
            selected.geometry.boundary.plot(ax=ax, color=None, edgecolor="gold",
                                            label="Selected countries", zorder=2)
    ...
    def generate_grid(
        ...
        if bbox is None:
            if radial_clip is None:
                bbox = world[world.name.isin(countries)].dissolve().buffer(buffer * 1000.)\
                           .to_crs("EPSG:4326").bounds.to_numpy()[0]
                bbox = [bbox[1], bbox[0], bbox[3], bbox[2]]
            else:
                bbox = bounding_box_from_radius(radial_clip[0], radial_clip[1], radial_clip[2],
                                                local_crs_epsg)
        ...
        if radial_clip is not None:
            coords = clip_to_radius(coords, radial_clip[0], radial_clip[1], radial_clip[2],
                                    search_radius, local_crs_epsg)
        if show:
            self.plot_grid(coords, countries, bbox, )
        if save_to is not None:
            coords.to_csv(save_to, float_format="%.5f", index=False,
...
def clip_to_countries(
    ...
    countries_ = countries_.dissolve().buffer(buffer * 1000.)[0]
    if search_radius is None:
        ...
        coords_ = coords.buffer(search_radius * 1000.)
...
def clip_to_bbox(
    ...
    if search_radius is None:
        ...
        coords_ = coords.to_crs(f"EPSG:{local_crs_epsg}")\
                        .buffer(search_radius * 1000.)
...
def buffer_bounding_box(
    ...
    """
    bbox_ = Polygon([(bbox[1], bbox[0]), (bbox[1], bbox[2]), (bbox[3], bbox[2]),
                     (bbox[3], bbox[0])])
    bbox_ = gpd.GeoSeries(bbox_).set_crs("EPSG:4326").to_crs(f"EPSG:{local_crs_epsg}")\
                .buffer(buffer * 1000.)
    return bbox_
...
def bounding_box_from_radius(
    ...
    search_radius = gpd.GeoSeries(center).set_crs("EPSG:4326").to_crs(f"EPSG:{local_crs_epsg}")\
                        .buffer(radius * 1000.)
...
def clip_to_radius(
    ...
    """
    center = Point(longitude, latitude)
    radius_ = gpd.GeoSeries(center).set_crs("EPSG:4326").to_crs(f"EPSG:{local_crs_epsg}")\
...
def parse_options():
    ...
    parser.add_argument("--bbox", metavar="<min_lat,min_lon,max_lat,max_lon>", dest="bbox",
                        action="store", type=str, required=False, default=None,
                        help="Specify a bounding box to search. Can be used in conjunction with "
                             "--countries")
    parser.add_argument("--countries", metavar="<country>[,<country>[,...]]", dest="countries",
                        action="store", type=str, required=False, default=None,
    ...
    parser.add_argument("--radial-clip", metavar="<lat,lon,radius_km>", dest="radial_clip",
                        action="store", type=str, required=False, default=None,
    ...
    parser.add_argument("--list-countries", dest="list_countries", action="store_true",
                        required=False, help="List the country names that can be used for the "
                                             "--countries option.")
    parser.add_argument("--buffer", metavar="<buffer>", dest="buffer", action="store", type=float,
    ...
    parser.add_argument("--search-radius", metavar="<radius>", dest="search_radius", action="store",
                        type=float, required=False, default=25.,
                        help="Specify the radial search limit around each grid point in "
                             "kilometers.")
    parser.add_argument("--local-crs-epsg", metavar="<EPSG>", dest="local_crs_epsg", action="store",
                        type=int, required=False, default=4087,
    ...
    parser.add_argument("--cache-dir", metavar="</path/to/dir>", dest="cache_dir", action="store",
                        type=str, required=False, default=None,
                        help="Specify a directory to use for caching downloaded boundary files.")
    parser.add_argument("--show", dest="show", action="store_true", required=False,
                        help="Set this flag to show a plot of the grid.")
    parser.add_argument("-o", "--outfile", metavar="<path>", dest="outfile",
    ...
    def handle_options(options):
        ...
        if options.radial_clip is not None:
            options.radial_clip = list(map(lambda x: float(x.strip()),
                                           options.radial_clip.split(",")))
        ...
        if options.outfile is not None and os.path.exists(options.outfile):
            check = query_yes_no(f"The output file '{options.outfile}' already exists, results "
                                 "will be overwritten, do you want to continue?", "no")
            if check is False:
                print("Quitting...")
                sys.exit(0)
        return options
...
def main():
    ...
    if options.list_countries:
        ...
        sys.exit()
    grd.generate_grid(bbox=options.bbox, countries=options.countries,
                      radial_clip=options.radial_clip, buffer=options.buffer,
                      search_radius=options.search_radius, local_crs_epsg=options.local_crs_epsg,
                      save_to=options.outfile, show=options.show)
